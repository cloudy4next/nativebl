...
<div class="content">
    <div class="card mb-3">
        <div class="card-header">
            <h5 class="card-title mb-0">Edit Agency</h5>
        </div>
        <div class="card-body">
            <form name="user_form" action={{ route('agency_save') }} method="post" class="">
                ...
                <div class="mb-3">
	    			...
	    			<input class="form-control  " id="name" name="name" type="text" placeholder="Name" value="{{$agencyDetails['name']}}">
				</div>                                
				<div class="mb-3">
				    <label class="form-label" for="icon">Icon</label>
				    <input class="form-control  " id="icon" name="icon" type="text" placeholder="Icon" value="{{$agencyDetails['icon']}}">
					...
					<tbody>
						<tr>
							...
							@foreach ($mappedUserArray as $key => $user)
								...
								echo '<td>'. $userList[$user] . '<a class="btn btn-danger btn-sm" href="/agency/delete-agency-user-map/'.$key.'/'. $agencyDetails['id'] .'">
                ...
                <a href="{{ route('agency_list') }}" class="btn btn-secondary"> Cancel </a>
            </form>
        </div>
    </div>
</div>
